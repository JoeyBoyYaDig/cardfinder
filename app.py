...
def build_search_links(card, marketplace_keywords=None):
    ...
    return {
        "query": query,
        "tcgplayer": f"https://www.tcgplayer.com/search/pokemon/product?productLineName=pokemon&q={encoded}",
        ...
        "cardmarket": f"https://www.cardmarket.com/en/Pokemon/Products/Search?searchString={encoded}",
...
def fetch_ebay_sold_sales(search_query):
    ...
    app_id = os.getenv(EBAY_APP_ID_ENV)
    if app_id:
        payload = fetch_ebay_sold_sales_api(search_query, app_id)
...
def ebay_sales():
    ...
    try:
        ...
        return jsonify(
            {
                "status": "unavailable",
                "message": "eBay lookup unavailable. Add an eBay App ID for reliable sold prices.",
            }
        ), 502
